...
// so it is compiled into the bundle. migrate, seed and reset-demo are separate
// entries so the production image can run them without tsx.
export default defineConfig({
  entry: {
    server: 'src/server.ts',
    migrate: 'src/db/migrate.ts',
    seed: 'src/db/seed.ts',
    'reset-demo': 'src/db/reset-demo.ts',
  },
  format: 'esm',
  platform: 'node',
  target: 'node22',
